...
module.exports = {
    ...
    exec: async (msg, args) => {
        ...
        if ((!music.player || !music.player.playing) && missingPerms.length)
            return msg.channel.send(`❌ I need ${missingPerms.length > 1 ? "these" : "this"} permission${missingPerms.length > 1 ? "s" : ""} on your voice channel: ${missingPerms.map(x => `\`${x}\``).join(", ")}.`);

        if (!music.node || !music.node.connected)
            return msg.channel.send("❌ Lavalink node not connected.");

        const query = args.join(" ") || getAttachmentURL(msg);
        if (!query) return msg.channel.send("❌ Missing args.");
        ...
        try {
            ...
            if (loadType === "PLAYLIST_LOADED") {
                ...
                if (music.player && music.player.playing)
                    msg.channel.send(`✅ **${track.info.title}** added to the queue.`);
            }
