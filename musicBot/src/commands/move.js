...
module.exports = {
    ...
    exec: async (msg, args) => {
        ...
        if (from === null || to === null)
            return msg.channel.send(`❌ Missing args. Example usage e.g. \`${msg.client.prefix}move 2 1\``);

        if (from === to || (isNaN(from) || from < 1 || from > music.queue.length) || (isNaN(to) || to < 1 || to > music.queue.length))
            return msg.channel.send("❌ Number is invalid or exceeds queue length.");

        const moved = music.queue[from - 1];

        util.moveArrayElement(music.queue, from - 1, to - 1);

        msg.channel.send(`✅ Moved **${moved.info.title}** to \`${to}\`.`);
    }
};
